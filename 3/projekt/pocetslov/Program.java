...
class Analyzátor { // Ve zdrojovém souboru java může mít pouze jedna třída specifikátor public
	...
	void běh() throws UnsupportedEncodingException {		
		...
		java.util.ArrayList<Dvojice> seznam = new java.util.ArrayList<Dvojice>();
		
		while(vstup.hasNextLine()){ // Čti vstup
			String řádka = vstup.nextLine();
			řádka = řádka.trim(); // odstraň okrajové mezery
			analyzujAUložVýsledky(řádka, seznam);
		}
		výstup.println(slovník); // Vypiš výsledky
		výstup.println(seznam);
		výstup.close();
	}		
	
	void přidej (String slovo, java.util.ArrayList<Dvojice> seznam){
		Dvojice pár = new Dvojice(slovo);
		int index = seznam.indexOf(pár);
		System.out.println(index);
		if(index<0){
			seznam.add(pár);
		} else {
			pár = seznam.get(index);
			pár.dalšíVýskyt();
		}
	}
	
	void analyzujAUložVýsledky(String řádka, java.util.ArrayList<Dvojice> seznam){
